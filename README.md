...
Автоудаление спам-сообщений настраивается не через env, а отдельно для каждого чата командой `/setspamdelete`.
...
| `/resetcounters <chat_id> [force=on]` | Сбросить дневные spam-счётчики. Без `force=on` доступно только в test mode. |
...
7. Каждое срабатывание пишется в `action_log`.
8. Если для чата включён `spam_delete_ttl_sec`, исходное spam-сообщение удаляется через заданное число секунд.
